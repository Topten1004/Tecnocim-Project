      ...
      <div class="content">
        <div class="row">
          ...
          <div class="col-6">
            ...
            <Select2
              v-model="Entidad"
              :options="Entidades.map(Entidad => { return {id: parseInt(Entidad.id), text: Entidad.nombre }; })"
              class="select-two form-control"
              @select="onChangeEntidad($event)"
              required
            ></Select2>
        ...
        <div class="row">
          <div class="col-12">
            <label for="s-cirbe" class="form-label">operación asociada </label>
            <select 
              name="s-cirbe"
              class="form-control"
              v-model="id_cirbe"
              :disabled="!Cirbes.length"
            >
              <option value="">---</option>
              <option
                  v-for="_cirbe in Cirbes"
                  v-bind:value="_cirbe.id"
                  v-bind:key="_cirbe.id"
                >
                  {{ _cirbe.entidad_nombre ?? _cirbe.entidad.nombre }} - {{ _cirbe.operacion }}
              </option>
            </select>
          </div>
        </div>
        ...
        <div class="row">
          ...
          <div class="col-6 m-auto">
            <div v-bind:class="{ invisible : !mensajeError }"><p class="text-danger m-auto" style="font-size: 0.8rem;">{{ mensajeError }}&nbsp;</p></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    ShowForm(id) {
      this.LoadContrato();
      this.isCirbe(true).then(() => {
        if(this.CurrentContrato?.contrato?.operacion) {
          this.id_cirbe = this.CurrentContrato.contrato.operacion.id;
        }
      });
      this.$BlockPop();
...
if(!this.Cuenta.startsWith("52"))
{
       for (let i = 0; i < _contratos.length; i ++) {          
          if(_contratos[i].cuenta.startsWith('52')  )
          {
            ...
            if (CurrentContrato.contrato != null && this.getCuentaAsociada(CurrentContrato) == _contratos[i].cuenta) {
      ...
      if (CurrentContrato.contrato != null) {
        ...
        this.Entidad = CurrentContrato.contrato.entidad.toString();
    ...
    async SaveContrato() {
      ...
      if(this.id_cirbe) {
        form.append("id_cirbe" ,this.id_cirbe);
      }
      ...
      await this.axios
        ...
        .catch((err) => {
          ...
          if(err.response && 'data' in err.response && typeof err.response.data == 'object' && 'error' in err.response.data) {
            this.mensajeError = err.response.data.error.error;
          } else {
            this.mensajeError = 'Ocurrió un error';
          }
        }); 
    },
    getCuentaAsociada(contrato) {
      return contrato.contrato == null ? "" : (contrato.contrato.cuentas[0].filter(c => c != contrato.cuenta))[0];
    },
    onChangeEntidad($event) {
      if($event.id == 0) {
        this.ProductoPrevio = this.Producto;
        this.Producto = 'excepcion';
      } else {
        if(this.ProductoPrevio) {
          this.Producto = this.ProductoPrevio;
          this.ProductoPrevio = '';
        }
      }
      this.isCirbe();
    ...
    async LoadCirbes(current = false) {
      this.axios.get("/core/cirbe_sin_contrato/", {
        params: {
          CIF: localStorage.getItem('company'),
          inicio: this.FechaInicio,
          vencimiento: this.FechaFin,
          entidad: this.Entidad
        }
      }).then((r) => {
        let cirbes = [];
        const operacion = this.CurrentContrato?.contrato?.operacion?.operacion;
        if(current && operacion) {
          cirbes.push(this.CurrentContrato.contrato.operacion);
        }
    ...
    async isCirbe(current = false) {
      if(
        ...
        this.id_cirbe = '';
        return this.LoadCirbes(current);
      }
