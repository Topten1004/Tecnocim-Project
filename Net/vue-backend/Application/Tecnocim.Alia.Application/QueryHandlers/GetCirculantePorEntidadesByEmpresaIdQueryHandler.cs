...
public class GetCirculantePorEntidadesByEmpresaIdQueryHandler : IRequestHandler<GetCirculantePorEntidadesByEmpresaIdQuery, GenericResult<TotalEntidadesResponse>>
{
    private readonly ILogger<GetCirculantePorEntidadesByEmpresaIdQueryHandler> _logger;
    private readonly IServiceProvider _serviceProvider;
    private readonly IMapper _mapper;

    public GetCirculantePorEntidadesByEmpresaIdQueryHandler(
        ILogger<GetCirculantePorEntidadesByEmpresaIdQueryHandler> logger,
    ...
    public async Task<GenericResult<TotalEntidadesResponse>> Handle(GetCirculantePorEntidadesByEmpresaIdQuery request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var tiposToExclude = new List<string> { "exception", "aval", "prestamo", "hipoteca", "leasing" };
            ...
            var pools = await unitOfWork.PoolRepository.GetIncludeAsync(x => x, x => !x.Deleted.HasValue
                && x.Cuenta.StartsWith("52") && x.Documento != null && x.Documento.EmpresaId == request.EmpresaId,
            ...
            var contratos = await unitOfWork.ContratoRepository.GetIncludeAsync(x => x, x => !x.Deleted.HasValue && x.Vencimiento > nowDateOnly
                && x.EquivalenciasProducto != null && !tiposToExclude.Contains(x.EquivalenciasProducto.Tipo),
                null, x => x.Include(t => t.EquivalenciasEntidad).Include(t => t.EquivalenciasMoneda).Include(t => t.EquivalenciasProducto)
                .Include(t => t.Pools).ThenInclude(t => t.Documento));
        ...
        {
            var message = $"Error al obtener el circulante por entidades para la empresa con id: {request.EmpresaId}";
